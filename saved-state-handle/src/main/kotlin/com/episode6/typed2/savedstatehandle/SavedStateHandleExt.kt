...
fun <T> TypedSavedStateHandle.property(key: BundleKey<T, *>): PropertyDelegate<T> = PropertyDelegate(
  get = { get(key) },
  set = { set(key, it) },
)
fun <T> SavedStateHandle.property(key: BundleKey<T, *>): PropertyDelegate<T> = typed().property(key)
