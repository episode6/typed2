...
class SavedStateProviderTest {
  ...
  @Test fun testClearSavedStateProvider() {
    savedStateHandle.setSavedStateProvider(Keys.testData) { TestData("Something") }
    savedStateHandle.clearSavedStateProvider(Keys.testData)

    savedStateHandle.savedStateProvider().saveState()

    assertThat(savedStateHandle).hasNoTestData()
  }
}
...
private fun Assert<SavedStateHandle>.hasNoTestData() = transform { it.get<Bundle>("testData") }.isNull()
