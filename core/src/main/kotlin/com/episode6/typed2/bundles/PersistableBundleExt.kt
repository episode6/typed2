...
public class TypedPersistableBundle(private val delegate: PersistableBundle) : PersistableBundleValueGetter, PersistableBundleValueSetter {
...
public fun <T> PersistableBundle.property(key: PersistableBundleKey<T, *>): DelegateProperty<T> = typed().property(key)
public fun <T> TypedPersistableBundle.property(key: AsyncPersistableBundleKey<T, *>, scope: CoroutineScope): DelegateProperty<T?> =
  DelegateProperty(mutableStateFlow(key, scope))
public fun <T> PersistableBundle.property(key: AsyncPersistableBundleKey<T, *>, scope: CoroutineScope): DelegateProperty<T?> = typed().property(key, scope)

public inline fun <T> TypedPersistableBundle.update(key: PersistableBundleKey<T, *>, reducer: (T)->T) {
  set(key, reducer(get(key)))
}

public suspend inline fun <T> TypedPersistableBundle.update(key: AsyncPersistableBundleKey<T, *>, reducer: (T)->T) {
  set(key, reducer(get(key)))
}
