...
public fun <T> Intent.setExtra(key: BundleKey<T, *>, value: T) = putExtras(Bundle().apply { set(key, value) })
public fun Intent.removeExtra(key: BundleKey<*, *>) = removeExtra(key.name)
public suspend fun <T> Intent.getExtra(key: AsyncBundleKey<T, *>): T = (extras ?: Bundle()).typed().get(key)
public suspend fun <T> Intent.setExtra(key: AsyncBundleKey<T, *>, value: T) = putExtras(Bundle().apply { set(key, value) })
public fun Intent.removeExtra(key: AsyncBundleKey<*, *>) = removeExtra(key.name)
