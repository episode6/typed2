...
fun <T> TypedBundle.property(key: BundleKey<T, *>): PropertyDelegate<T> = PropertyDelegate(
  get = { get(key) },
  set = { set(key, it) }
)
fun <T> Bundle.property(key: BundleKey<T, *>): PropertyDelegate<T> = typed().property(key)
